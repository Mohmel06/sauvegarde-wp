...
<?php
$post = get_post();
$prix_de_vente = get_field('prix_de_vente');
$code_ean = get_field('code_ean');
$reduction = get_field('reduction');
$prix_reduit = $prix_de_vente * (100-$reduction) / 100;
...
$categories_id = get_field('categories_produit');
$categories_array = [];
...
<main id="content" <?php post_class( 'site-main' ); ?> role="main">
	...
	<div class="page-content">
		...
		<p>Prix : <?=number_format($prix_de_vente,2,',',' ')?> € (<?=$taux_tva?>%)</p>
		<?php
			if($reduction>0): ?>
			...
			<?php endif; ?>
				...
				<p>Montant TVA : <?=number_format($montant_tva,2,',',' ')?> €</p>
